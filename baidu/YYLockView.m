...
-(void)_setup
{
    ...
    for (int i=0; i<9; i++) {
        ...
        //2.设置按钮的状态背景
        [btn setImage:[UIImage imageNamed:@"lock_off"]  forState:UIControlStateNormal];
        [btn setImage:[UIImage imageNamed:@"lock_on"]  forState:UIControlStateSelected];
        //3.把按钮添加到视图中
        [self  addSubview:btn];
        //4.禁止按钮的点击事件
...
{
    CGPoint starPoint=[self _getCurrentPoint:touches];
    UIButton *btn=[self _getCurrentBtnWithPoint:starPoint];
...
{
    CGPoint movePoint=[self _getCurrentPoint:touches];
    UIButton *btn=[self _getCurrentBtnWithPoint:movePoint];
    //存储按钮
    //已经连过的按钮，不可再连
    if (btn && btn.selected != YES) {
        //设置按钮的选中状态
...
//对功能点进行封装
-(CGPoint)_getCurrentPoint:(NSSet *)touches
{
    UITouch *touch=[touches anyObject];
    CGPoint point=[touch locationInView:touch.view];
    return point;
}
-(UIButton *)_getCurrentBtnWithPoint:(CGPoint)point
{
